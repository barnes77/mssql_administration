...
Aim: Gather information about AG groups of DBAs in a domain via Net User
Corrections:
...
IF OBJECT_ID('#dba_ad_xp_refined','U') IS NOT NULL BEGIN DROP TABLE #dba_ad_xp_refined; END
...
CREATE TABLE #dba_ad_xp_refined (
	[line] varchar(max)
);
...
DECLARE @ad_prefix varchar(20), @line varchar(max), @ad_groups_no int, @ad_acc varchar(50), @query1 varchar(max);
...
BEGIN
	...
	SET @query1 = 'exec master..xp_cmdshell ''net user /domain '+@ad_prefix+@ad_acc+'''';
	INSERT INTO #dba_ad_xp
	exec (@query1);
 
	--Include data about AD user and AD groups only
	INSERT INTO #dba_ad_xp_refined
	SELECT * FROM #dba_ad_xp WHERE [line] LIKE '%*%' OR [line] LIKE '%User name%';
	--Get unrefined list of AD groups into one variable per AD account
	SET @line = '';
	SELECT @line = @line + [line] FROM #dba_ad_xp_refined;
	SELECT @line = LTRIM(RTRIM(REPLACE(REPLACE(@line,'Global Group memberships',''),'User name','')));
	TRUNCATE TABLE #dba_ad_xp;
	TRUNCATE TABLE #dba_ad_xp_refined;
 
	--Get number of AD groups per AD account
	SET @ad_groups_no = LEN(@line) - LEN(REPLACE(@line,'*',''))
	--Split string of net user results into separate rows
	SELECT @line = @line+'*'
	DECLARE @i int = 0
	WHILE @i < @ad_groups_no
	BEGIN
		SELECT @i = @i + 1
		INSERT INTO #dba_ad_acc_group
		SELECT @ad_acc, SUBSTRING(@line,CHARINDEX('*',@line)+1,CHARINDEX('*',@line,CHARINDEX('*',@line)+1)-CHARINDEX('*',@line)-1)
		SELECT @line = SUBSTRING(@line,CHARINDEX('*',@line,CHARINDEX('*',@line)+1),len(@line)-CHARINDEX('*',@line,CHARINDEX('*',@line)));
	END
