...
SELECT DB_NAME() AS [database_name]
	 ,dbpri1.[name] AS [user_name]
	 ,dper.state_desc+'': ''+dper.permission_name+'' on ''+sobj.[name] COLLATE Latin1_General_CI_AS +'' (''+sobj.[type_desc] COLLATE Latin1_General_CI_AS +'')'' AS db_permission
	 ,dbpri1.[sid] AS user_sid
	 ,dbpri1.[type_desc] AS user_type
FROM sys.database_permissions AS dper
LEFT JOIN sys.objects AS sobj ON dper.major_id = sobj.object_id
LEFT JOIN sys.database_principals AS dbpri1 ON dper.grantee_principal_id = dbpri1.principal_id
LEFT JOIN sys.database_principals AS dbpri2 ON dper.grantor_principal_id = dbpri2.principal_id
...
DECLARE db_crsr CURSOR LOCAL FAST_FORWARD FOR
	...
	AND is_online + @include_emergency >= 1 AND [name] Like '%'+@db_name_pattern+'%' AND is_offline <> 1 AND is_restoring <> 1
...
WHERE is_user_db + @include_system_db < 1 OR is_bulk + is_full + @include_simple_rec < 1 OR is_simple + is_full + @include_bulk_rec < 1 OR is_simple + is_bulk + @include_full_rec < 1 OR is_restricted_user + is_multi_user + @include_single_user < 1
	OR is_single_user + is_multi_user + @include_restricted_user < 1 OR is_single_user + is_restricted_user + @include_multi_user < 1 OR is_online + @include_emergency < 1
	OR [name] NOT Like '%'+@db_name_pattern+'%' OR is_offline = 1 OR is_restoring = 1
