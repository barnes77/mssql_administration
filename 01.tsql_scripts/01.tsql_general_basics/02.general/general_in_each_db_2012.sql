...
@include_ag_secondary tinyint, @query nvarchar(2000);
...
SELECT
	sd.[name]
	,CASE WHEN sd.database_id > 4 THEN 1 ELSE 0 END AS [is_user_db]
	,CASE WHEN sd.recovery_model = 3 THEN 1 ELSE 0 END AS [is_simple]
	,CASE WHEN sd.recovery_model = 2 THEN 1 ELSE 0 END AS [is_bulk]
	,CASE WHEN sd.recovery_model = 1 THEN 1 ELSE 0 END AS [is_full]
	,CASE WHEN sd.user_access = 0 THEN 1 ELSE 0 END AS [is_multi_user]
	,CASE WHEN sd.user_access = 1 THEN 1 ELSE 0 END AS [is_single_user]
	,CASE WHEN sd.user_access = 2 THEN 1 ELSE 0 END AS [is_restricted_user]
	,CASE WHEN sd.is_read_only = 1 THEN 1 ELSE 0 END AS [is_read_only]
	,CASE WHEN sd.[state] = 0 THEN 1 ELSE 0 END AS [is_online]
	,CASE WHEN sd.[state] = 5 THEN 1 ELSE 0 END AS [is_emergency]
	,CASE WHEN sd.[state] IN (6,10) THEN 1 ELSE 0 END AS [is_offline]
	,CASE WHEN sd.[state] = 1 THEN 1 ELSE 0 END AS [is_restoring]
	,CASE WHEN sd.containment = 1 THEN 1 ELSE 0 END AS [is_contained]
	,CASE WHEN sd.containment = 0 THEN 1 ELSE 0 END AS [is_not_contained]
	...
	,CASE WHEN dhars.[role] = 1 THEN 1 ELSE 0 END AS is_ag_primary
	,CASE WHEN dhars.[role] = 2 THEN 1 ELSE 0 END AS is_ag_secondary
FROM sys.databases AS sd
LEFT JOIN sys.availability_databases_cluster AS adc ON sd.[name] = adc.database_name
LEFT JOIN sys.dm_hadr_availability_replica_states AS dhars ON dhars.group_id = adc.group_id;
